...
# 데이터베이스 테이블 생성
# models.Base.metadata.create_all(bind=engine) # Alembic으로 관리하므로 주석 처리 또는 삭제
...
def test_naver_api():
    """
    .env 파일에 설정된 키로 네이버 검색 API를 직접 호출하여 연동 상태를 테스트합니다.
    """
    from .service import naverMapService
    logging.info("네이버 API 연동 테스트 시작...")
    test_query = "강남역 맛집"
    results = naverMapService.search_local_places(query=test_query, display=5)
    
    if results:
        logging.info("네이버 API 연동 성공!")
        return {"status": "성공", "query": test_query, "count": len(results), "items": results}
    else:
        logging.error("네이버 API 연동 실패!")
        raise HTTPException(status_code=500, detail="네이버 API 연동에 실패했습니다. .env 파일의 NAVER_CLIENT_ID와 NAVER_CLIENT_SECRET을 확인하세요.")
