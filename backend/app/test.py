...
# 테스트 실행을 위해 프로젝트 루트의 .env 파일을 명시적으로 로드합니다.
...
from . import crud
from .service import naverMapService # 기존 네이버 맵 서비스 사용
...
def run_test_without_db():
    ...
    try:
        ...
        # 네이버 API 원본 응답을 직접 확인
        import requests
        naver_id = os.getenv("NAVER_CLIENT_ID")
        naver_secret = os.getenv("NAVER_CLIENT_SECRET")
        
        headers = {"X-Naver-Client-Id": naver_id, "X-Naver-Client-Secret": naver_secret}
        params = {"query": "강남역 맛집", "display": 3}
        
        print("   -> 네이버 API 직접 호출 테스트...")
        response = requests.get("https://openapi.naver.com/v1/search/local.json", headers=headers, params=params)
        print(f"   -> HTTP 상태 코드: {response.status_code}")
        
        if response.status_code == 200:
            api_result = response.json()
            print(f"   -> 전체 응답: {json.dumps(api_result, indent=2, ensure_ascii=False)}")
            candidates = api_result.get("items", [])
        else:
            print(f"   -> 오류 응답: {response.text}")
            candidates = []
        
        print(f"   -> 최종 검색 결과 개수: {len(candidates)}개")
